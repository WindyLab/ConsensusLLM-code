...
class Vector2dDebate(Template):
  ...
  def generate_agents(self, simulation_ind):
    agents = []
    position = np.array([[20, 20], [80, 20], [50, 80]]) + np.random.randint(-10, 10, size=(self._n_agents, 2))
    # position = np.random.randint(0, 100, size=(self._n_agents, 2))
  ...
  def update_record(self, record, agent_contexts, simulation_ind, agents):
    record[tuple(tuple(pos) for pos in self._positions[simulation_ind])] = agent_contexts
    self._trajectory['pos'][simulation_ind] = [agent.trajectory for agent in agents]
    self._trajectory['target'][simulation_ind] = [agent.target_trajectory for agent in agents]
